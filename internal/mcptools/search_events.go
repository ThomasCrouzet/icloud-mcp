...
func newSearchEventsTool(defaultLoc *time.Location) mcp.Tool {
	return mcp.NewTool("search_events",
		mcp.WithDescription("Searches events over a range. Recurrence expands by default; occurrences include recurrenceId for occurrence updates. Results are sorted, capped at 400 and 256 KiB, and may report truncation or partial calendar failures."),
		...
		mcp.WithString("calendar", mcp.Description("One calendar path; all calendars when omitted")),
		mcp.WithString("calendars", mcp.Description("Up to 10 comma-separated paths; merged with calendar")),
		mcp.WithString("query", mcp.MaxLength(icloud.MaxQueryLen), mcp.Description("Case-insensitive title/location/notes filter; max 200 UTF-8 bytes")),
		mcp.WithString("uid", mcp.MaxLength(icloud.MaxUIDLen), mcp.Description("Exact UID filter; max 255 UTF-8 bytes")),
		mcp.WithString("status", mcp.Enum("TENTATIVE", "CONFIRMED", "CANCELLED"), mcp.Description("Status filter")),
		mcp.WithBoolean("all_day", mcp.Description("Keep all-day (true) or timed (false) events")),
		mcp.WithBoolean("include_cancelled", mcp.DefaultBool(true), mcp.Description("Include CANCELLED events")),
		mcp.WithBoolean("busy_only", mcp.DefaultBool(false), mcp.Description("Exclude TRANSPARENT events")),
		mcp.WithBoolean("compact", mcp.DefaultBool(false), mcp.Description("Omit notes; recommended for wide ranges")),
		mcp.WithBoolean("expand_recurrence", mcp.DefaultBool(true), mcp.Description("Expand recurring occurrences")),
		mcp.WithInteger("limit", mcp.DefaultNumber(100), mcp.Min(1), mcp.Max(icloud.MaxResults), mcp.Description("Page size; max 400")),
